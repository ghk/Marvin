...
public class WebServer implements Runnable{
    ...
    static void prepare(CameraPreviewHolder previewHolder){    
    	/* start worker threads */
        for (int i = 0; i < workers; ++i) {
            Worker w = new Worker(previewHolder);
    ...
    public WebServer(CameraPreviewHolder previewHolder) {
        ...
        try{
	        ...
	        while (isRunning) {	
	            ...
	            synchronized (threads) {
	                if (threads.isEmpty()) {
	                    Worker ws = new Worker(this.previewHolder);
	                    ws.setSocket(s);
        	...
        	Log.e(MarvinActivity.TAG, e.getMessage());
        	e.printStackTrace();
        	return;
        }
    }
...
class Worker extends WebServer implements HttpConstants, Runnable {
    ...
    Worker(CameraPreviewHolder previewHolder) {
    	super(previewHolder);
    ...
    void handleClient() throws IOException {
    	Log.d(MarvinActivity.TAG, "handling client");
        InputStream is = new BufferedInputStream(s.getInputStream());
        PrintStream ps = new PrintStream(s.getOutputStream(), false, "ascii");
...
outerloop:
    ...
    private void executeStreaming(PrintStream ps) throws IOException
    {
        int rCode = 0;
        ...
        ps.write(EOL);
        String ct = "multipart/x-mixed-replace; boundary=--testboundary";        
        ps.print("Content-type: " + ct);
        ps.write(EOL);
        ps.write(EOL);
        for(byte[] buffer = previewHolder.getCurrentBuffer(); buffer != null && buffer.length > 0; buffer = previewHolder.getCurrentBuffer()){
        	ps.print("\r\n--testboundary\r\nContent-Type: image/jpeg\r\nContent-Length:" + buffer.length + "\r\n\r\n");
        	ps.write(buffer);
        	if(hasWrite < 5){
        		hasWrite++;
        		FileOutputStream fos = new FileOutputStream(new File(root, "testtest"+hasWrite+".jpg"));
        		fos.write(buffer);
        		fos.close();
        	}
        	ps.flush();
